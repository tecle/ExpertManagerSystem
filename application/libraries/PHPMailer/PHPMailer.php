  ...
  */
  ...
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = '') {
    try {
      ...
      //$filename = basename($path);
      $path_1=mb_convert_encoding($path,'UTF-8' ,'GB2312,UTF-8');
        if (false === strpos($path_1, '/'))
            $filename = $this->EncodeHeader($path_1);
        else
            $filename = $this->EncodeHeader(substr($path_1, strrpos($path_1, '/') + 1));
