...
function addrInit(lobj, cobj, province, city) {
        ...
        for (var i = 0; i < where.length; i++) {
                ...
                oOption.text = where[i].loca;
                oOption.value = where[i].loca;
                if(where[i].loca == init_province)
                    oOption.selected = true;
                lobj.add(oOption);
        }
        var oOption = document.createElement("OPTION");
        oOption.text = where[0].locacity;
        oOption.value = where[0].locacity;
        cobj.add(oOption);
        if(init_city){
            select(lobj, cobj);
        }
...
function select(lobj, cobj) {
        cobj.length = 0;
        local = lobj.value;
  
        for (var i = 0; i < where.length; i++) {
                if (where[i].loca == local) {
                        break;
                }
        }
        var cities = (where[i].locacity).split("|");
        var oOption;
        for (var j = 0; j < cities.length; j++) {
                oOption = document.createElement("OPTION");
                oOption.text = cities[j];
                oOption.value = cities[j];
                if(cities[j] == init_city)
                    oOption.selected = true;
                cobj.add(oOption);
        }
}